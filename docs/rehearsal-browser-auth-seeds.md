...
Saving only the Chrome profile is not enough. Chrome cookies on macOS are encrypted with the `Chrome Safe Storage` item in the user's login keychain. A reusable seed must preserve both:
