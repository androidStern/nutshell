...
| permissions gate pre mode passes when every protected source reports needs_permission with fix text | test/fresh-install-rehearsal.test.ts | Rewritten as "permissions gate pre mode passes when notes gate on permission, podcasts report no library, and browser findings are guided". The v0.1.23 VM run (reports/permissions-pre-gate-v0.1.23-20260610.product-fail-frozen.json) proved the old per-source needs_permission requirement was a harness bug: Chrome's cookie store is not Full-Disk-Access-protected, so youtube/twitter legitimately pass pre-grant, and a fresh VM has no Apple Podcasts library, so podcasts honestly reports podcasts_db_missing (ready_empty). |
| permissions gate pre mode fails a protected source that reports fake-ready | test/fresh-install-rehearsal.test.ts | Rewritten as "permissions gate pre mode fails apple_notes or podcasts reporting fake-ready". The fake-ready failure only applies to the genuinely permission-gated sources (apple_notes, podcasts); browser sources can genuinely pass pre-grant, so twitter is no longer a valid fake-ready subject. |

### test/fresh-install-rehearsal.test.ts (permissions-pre and signed-out contract corrections, 2026-06-10)

| Test | Traces to | Verdict |
| --- | --- | --- |
| permissions gate pre mode passes when notes gate on permission, podcasts report no library, and browser findings are guided | gates doc "Permissions Gate" pre-state contract per source; honest-setup #25; v0.1.23 frozen evidence (Chrome cookies not FDA-protected; fresh VM has no Podcasts library) | anchored |
| permissions gate pre mode accepts podcasts needs_permission when a protected library exists | gates doc pre-state contract (podcasts: needs_permission-or-no-library; FDA-related podcasts findings only appear when a protected library exists) | anchored |
| permissions gate pre mode fails apple_notes or podcasts reporting fake-ready | honest-setup #25; truthful-baseline §1 (no quiet fake success); gates doc pre-state contract (a pre-grant pass with zero findings for notes/podcasts is fake-ready) | anchored |
| permissions gate pre mode does not fail youtube or twitter passing cleanly before grants | gates doc pre-state contract (browser sources are not FDA-gated and may pass pre-grant); regression: v0.1.23 permissions-pre gate failed honest browser passes as missing needs_permission | anchored |
| permissions gate pre mode fails a browser source finding that carries no guidance | gates doc pre-state contract (any browser-source finding must carry guidance); honest-setup #10 | anchored |
